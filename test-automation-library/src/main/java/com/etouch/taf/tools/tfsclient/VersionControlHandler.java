...
import com.etouch.taf.tools.tfsclient.event.CustomGetEventListener;
import com.etouch.taf.tools.tfsclient.event.CustomGetOperationCompletedListener;
import com.etouch.taf.tools.tfsclient.event.CustomGetOperationStartedListener;
...
public class VersionControlHandler {
	...
	public static Workspace createAndMapWorkspace(final TFSTeamProjectCollection tpc) {
		final String workspaceName = ConsoleConnection.WORKSPACE_NAME; 
		...
		if (workspace == null) {
			workspace = tpc.getVersionControlClient().createWorkspace(null, workspaceName, "Workspace comment", WorkspaceLocation.SERVER, null,
					WorkspacePermissionProfile.getPrivateProfile());

			// Map the workspace
			WorkingFolder workingFolder = new WorkingFolder(ConsoleConnection.MAPPING_SERVER_PATH, LocalPath.canonicalize(ConsoleConnection.MAPPING_LOCAL_PATH));
	...
	public static int checkoutFromTFS(final Workspace workspace, File file, final String comment) {
		...
		cs = workspace.pendEdit(new ItemSpec[] { fileSpec }, LockLevel.UNCHANGED, ENCODING, GetOptions.NONE, PendChangesOptions.GET_LATEST_ON_CHECKOUT);
		return cs;
	}
	...
	public static void displayFileHistory(final Workspace workspace, final File file) {
		...
		Changeset[] cs = null;
		try {
			cs = workspace.queryHistory(file.getCanonicalPath(), LatestVersionSpec.INSTANCE, 0, RecursionType.NONE, null, null, LatestVersionSpec.INSTANCE, Integer.MAX_VALUE,
					true, false, false, false);
		} catch (ServerPathFormatException e) {

			log.debug("ServerPathFormatException", e);
		} catch (IOException e) {
			log.debug("IOException", e);
		}

		for (Changeset changeset : cs) {
			log.debug("CS#:" + changeset.getChangesetID() + " ChangeType:" + changeset.getChanges()[0].getChangeType().toUIString(true, changeset.getChanges()[0]));
