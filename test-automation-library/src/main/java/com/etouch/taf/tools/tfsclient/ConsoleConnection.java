...
public class ConsoleConnection {
	...
	/** The password. */
	protected static TafPassword PASSWORD = tfsConfig.getPassword();

	/**
	 * A team project name (without the leading "$/") where files, work items,
	 * and builds can be created and modified.
	 */
	protected static final String PROJECT_NAME = tfsConfig.getProjectName();
	 ...
	 * path below.
	 */
	protected static final String MAPPING_SERVER_PATH = tfsConfig.getMappingServerPath();
	 ...
	 */
	protected static final String MAPPING_LOCAL_PATH = tfsConfig.getMappingLocalPath();

	/** The workspace name. */
	protected static final String WORKSPACE_NAME = tfsConfig.getWorkspaceName();

	/** The max reconnections. */
	protected static final int MAX_RECONNECTIONS = tfsConfig.getMaxReconections();
	...
	public static TFSTeamProjectCollection connectToTFS() throws Exception {
		...
		if (System.getProperty("com.microsoft.tfs.jni.native.base-directory") == null) {
			...
			log.debug(System.getProperty("com.microsoft.tfs.jni.native.base-directory"));
		}
		...
		if ((USERNAME == null || USERNAME.length() == 0) && CredentialsUtils.supportsDefaultCredentials()) {
			...
			credentials = new UsernamePasswordCredentials(USERNAME, PASSWORD.toString());
		}
