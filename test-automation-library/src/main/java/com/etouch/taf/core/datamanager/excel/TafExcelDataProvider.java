...
public class TafExcelDataProvider {
	 ...
	 *             the taf data provider exception
	 */
	private static void resolveExcelFiles(Class<?> callingTestClass) throws TafDataProviderException {
			...
			if (!dataContainer.hasDataFile(excel[0])) {
				synchronized (callingTestClass) {
					if (!dataContainer.hasDataFile(excel[0])) {
						...
						// keyword Driven XLSX filePath
						if (TestBedManagerConfiguration.getInstance().getKdConfig().isExecute()
								&& dataContainer.isDataFileExists(excel[1])) {
							dataContainer.addDataFile(excel[0], excel[1]);
						} else {
