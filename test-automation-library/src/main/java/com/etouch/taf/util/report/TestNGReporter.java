...
public class TestNGReporter implements IReporter {
	...
	/** The Constant SP3. */
	private static final String SP3 = SP2 + SP2;

	/** The Constant SP4. */
	private static final String SP4 = SP3 + SP3;

	private static Log log = LogUtil.getLog(TestNGReporter.class);

	/** The mClasses. */
	private Map<String, ITestClass> mClasses = Maps.newHashMap();
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
		mOutputDirectory = generateOutputDirectoryName(outputDirectory + File.separator + "old");
	...
	/**
	 * Overridable by subclasses to create different directory names (e.g. with
	 * timestamps).
	 * 
	...
	/**
	 * Generate the main index.html file that lists all the suites and their
	 * result
	 * 
	 * @param suites
	 *            the suites
	...
	private void generateIndex(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			suiteBuf.append("<td>" + passedTests + "</td>").append("<td>" + failedTests + "</td>").append("<td>" + skippedTests + "</td>").append("<td><a href='").append(name)
					.append("/").append(TESTNG_XML).append("'>Link").append("</a></td>").append("</tr>");

		}

		String cls = totalFailedTests > 0 ? "invocation-failed" : (totalPassedTests > 0 ? "invocation-passed" : "invocation-failed");
		sb.append("<tr align='center' class='").append(cls).append("'>").append("<td><em>Total</em></td>").append("<td><em>").append(totalPassedTests).append("</em></td>")
				.append("<td><em>").append(totalFailedTests).append("</em></td>").append("<td><em>").append(totalSkippedTests).append("</em></td>").append("<td>&nbsp;</td>")
				.append("</tr>\n");
		sb.append(suiteBuf);
		sb.append("</table>").append("</body></html>\n");

		Utils.writeFile(mOutputDirectory, "index.html", sb.toString());
	}
	...
	private void generateClasses(XmlSuite xmlSuite, ISuite suite) {
		...
		for (ITestClass tc : mClasses.values()) {
	...
	private String generateClass(ITestClass cls) {
		...
		ITestNGMethod[][] methods = new ITestNGMethod[][] { cls.getTestMethods(), cls.getBeforeClassMethods(), cls.getBeforeTestMethods(), cls.getAfterTestMethods(),
	...
	private void generateMethodsChronologically(XmlSuite xmlSuite, ISuite suite, String outputFileName, boolean alphabetical) {
		...
		for (IInvokedMethod iim : invokedMethods) {
			...
			table.setLength(0);
			if (!addedHeader) {
				table.append("<table border=\"1\">\n").append("<tr>").append("<th>Time</th>").append("<th>Delta (ms)</th>").append("<th>Suite<br>configuration</th>")
						.append("<th>Test<br>configuration</th>").append("<th>Class<br>configuration</th>").append("<th>Groups<br>configuration</th>")
						.append("<th>Method<br>configuration</th>").append("<th>Test<br>method</th>").append("<th>Thread</th>").append("<th>Instances</th>").append("</tr>\n");
				addedHeader = true;
			}
			...
			table.append("<tr bgcolor=\"" + createColor(tm) + "\">").append("  <td>").append(date).append("</td> ").append("  <td>").append(iim.getDate() - startDate)
					.append("</td> ").append(td(configurationSuiteMethod)).append(td(configurationTestMethod)).append(td(configurationClassMethod))
					.append(td(configurationGroupsMethod)).append(td(setUpOrTearDownMethod)).append(td(testMethod)).append("  <td>").append(tm.getId()).append("</td> ")
					.append("  <td>").append(instances).append("</td> ").append("</tr>\n");
	...
	private String createColor(ITestNGMethod tm) {
		...
		long[] rgb = { ((color & 0xff0000) >> 16) & 0xff, ((color & 0x00ff00) >> 8) & 0xff, color & 0xff };
		// Not too dark
	...
	private void ppp(String s) {
		CommonUtil.sop("[SuiteHTMLReporter] " + s);
	}
	...
	private void generateIndex(XmlSuite xmlSuite, ISuite sr) {
		StringBuilder index = new StringBuilder().append("<html><head><title>Results for " + sr.getName() + "</title></head>\n").append("<frameset cols=\"26%,74%\">\n")
	...
	private void generateTableOfContents(XmlSuite xmlSuite, ISuite suite) {
		...
		for (ISuiteResult sr : suiteResults.values()) {
			...
			for (ITestNGMethod tm : methods) {
				...
				mClasses.put(tc.getRealClass().getName(), tc);
			}
		}
		...
		tableOfContents.append("<body>\n").append("<h3><p align=\"center\">" + makeTitle(suite) + "</p></h3>\n").append("<table border='1' width='100%'>\n")
				.append("<tr valign='top'>\n").append("<td>").append(suiteResults.size()).append(" ").append(pluralize(suiteResults.size(), "test")).append("</td>\n")
				.append("<td>").append("<a target='mainFrame' href='").append(CLASSES).append("'>").append(mClasses.size() + " " + pluralize(mClasses.size(), "class"))
				...
				.append("&nbsp;&nbsp;<a target='mainFrame' href='").append(METHODS_NOT_RUN).append("'>not run (" + suite.getExcludedMethods().size() + ")</a>").append("</td>\n")
				.append("</tr>\n")

				.append("<tr>\n").append("<td><a target='mainFrame' href='").append(GROUPS).append("'>").append(groupCount + pluralize(groupCount, " group") + "</a></td>\n")
				.append("<td><a target='mainFrame' href='").append(REPORTER_OUTPUT).append("'>reporter output</a></td>\n").append("<td><a target='mainFrame' href='")
				.append(TESTNG_XML).append("'>testng.xml</a></td>\n").append("</tr>").append("</table>");

		//
		// Generate results for individual tests
		//

		// Order the results so we can show the failures first, then the skip
		// and
		...
		ISuiteResult[][] results = new ISuiteResult[][] { sortResults(redResults.values()), sortResults(yellowResults.values()), sortResults(greenResults.values()) };
		...
		for (int i = 0; i < colors.length; i++) {
			...
			for (ISuiteResult sr : r) {
				...
				generateSuiteResult(suiteName, sr, colors[i], tableOfContents, mOutputDirectory);
			}
		}
	...
	private String getOutputDirectory(XmlSuite xmlSuite) {
		return mOutputDirectory + File.separatorChar + xmlSuite.getName();
	}
	 ...
	 */
	private void generateSuiteResult(String suiteName, ISuiteResult sr, String cssClass, StringBuilder tableOfContents, String outputDirectory) {
		ITestContext tc = sr.getTestContext();
		int passed = tc.getPassedTests().size();
		int failed = tc.getFailedTests().size();
		int skipped = tc.getSkippedTests().size();
		...
		tableOfContents.append("\n<table width='100%' class='test-").append(cssClass).append("'>\n").append("<tr><td>\n").append("<table style='width: 100%'><tr>")
				.append("<td valign='top'>").append(suiteName).append(" (").append(passed).append("/").append(failed).append("/").append(skipped).append(")").append("</td>")
