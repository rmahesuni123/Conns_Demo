...
public class ConfigUtil {
	...
	private static String testWebEnvironment = tbMgrConfig.getWebConfig()
			.getTestEnv();
	
	/** The test mobile environment. */
	private static String testMobEnvironment = tbMgrConfig.getMobileConfig().getTestEnv();
	...
	public static boolean isLocalEnv(String testBedName) {
		...
		boolean result = false;
		if(TestBedManager.INSTANCE.getCurrentTestBeds().get(testBedName).getTestType().equalsIgnoreCase(TestTypes.WEB.name()))
		{
			if (TestEnvironmentType.LOCAL.getTestEnv().equals(testWebEnvironment)) {
				result = true;
			}
		}
		else
		{
			if (TestEnvironmentType.LOCAL.getTestEnv().equals(testMobEnvironment)) {
				result = true;
			}
		}
	...
	public static boolean isRemoteEnv(String testBedName) {
		...
		if(TestBedManager.INSTANCE.getCurrentTestBeds().get(testBedName).getTestType().equalsIgnoreCase(TestTypes.WEB.name()))
		{
			if (TestEnvironmentType.REMOTE.getTestEnv().equals(testWebEnvironment)) {
				result = true;
			}
		}
		else
		{
			if (TestEnvironmentType.REMOTE.getTestEnv().equals(testMobEnvironment)) {
				result = true;
			}
	...
	/**
	 * Checks if is experi test.
	 *
	 * @return true, if is experi test
	 */
	public static boolean isExperiTest() {
		boolean result = false;
		if (tbMgrConfig.getMobileConfig().getTool()
				.equalsIgnoreCase(TestToolType.EXPERITEST.getTestToolType())) {
			result = true;
		}
		return result;		
	}
	...
	public static Collection<String> getTestBeds(){
		...
		else if (ConfigUtil.isMobileTestTypeEnabled()){
			MobileConfig mobileConfig = TestBedManagerConfiguration.getInstance().getMobileConfig();
			testBedColls = Arrays.asList(mobileConfig.getCurrentTestBeds());
		}
